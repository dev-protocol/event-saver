...
import timerTrigger from '../../../policy-factory-create/index'
...
describe('timerTrigger', () => {
	...
	beforeEach(async () => {
		await clearData(con.connection, PolicyFactoryCreate)
	})
	it('Register data as many events as there are.', async () => {
		...
		const count = await getCount(con.connection, PolicyFactoryCreate)
		expect(count).toBe(3)
